...
export const deleteListing = async (req, res, next) => {
    ...
    if (req.user.id !== listing.userRef) {
        return next(errorHandler(401, 'You Can Onlu Delete your Listing'))
    }
    try {
...
export const updateListing = async (req, res, next) => {
    ...
    if (req.user.id !== listing.userRef) {
        return next(errorHandler(401, 'You Can Only Update Youre Own Listing'))
    }
...
export const getListings = async (req, res, next) => {
    try {
        ...
        const lisitings = await Listing.find({
            name: { $regex: searchTerm, $options: 'i' },//options i means No diffrence in sear between Capital or small letters 
            offer,
            furnished,
            parking,
            type,
